...
(
    ...
    hashedToken VARCHAR(255)             NOT NULL UNIQUE,
    created_at  TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP,
    version     BIGINT                   NOT NULL DEFAULT 0
);

CREATE INDEX IF NOT EXISTS idx_refresh_tokens_user_id ON user_service.refresh_tokens (user_id);
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_hashed_token ON user_service.refresh_tokens (hashedToken);
CREATE INDEX IF NOT EXISTS idx_refresh_tokens_expires_at ON user_service.refresh_tokens (expires_at);
